...
%Stream2Raven - Compute:
%Takes a log from a filestream and makes Raven compatible selection
%tables (.txt files) for each sound file within the log

%% Add sound files for each event into every log.event structure - uses code from 'Get Files' Log Action
...
cd(log.path(1:end-5));
mkdir('Raven Logs');
Rav_dir = [log.path(1:end-5) 'Raven Logs'];
